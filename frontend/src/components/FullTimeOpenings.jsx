...
const Internship = () => {
  ...
  const { data: jobs, loading, error } = useFetchOpportunities({
    type: "job",
    limit: 3,
  });
  ...
  return (
    ...
    <section
      id="job-section"
      className="py-12 px-4 sm:px-6 lg:px-8 bg-gray-100 pt-20"
    >
      {/* ==============================
          🏷️ SECTION HEADER
      ...
      <div className="max-w-7xl mx-auto grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
        ...
        ) : /* ✅ If data is fetched successfully and internships exist */ jobs.length > 0 ? (
          jobs.map((job) => (
            <OpportunityCard
              key={job.id}
              id={job.id}
              ...
              logo={job.logo || "/logos/default-logo.png"}
              type={job.type || "Internship"}
              location={job.location || "Remote"}
              postedDate={job.posted_at}
              closingDate={job.deadline}
              postedOn={job.source || "Careers Page"}
            />
          ))
        ) : (
